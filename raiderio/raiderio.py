...
class RaiderIO(commands.Cog):
    ...
    async def create_affixes_embed(self, json: json, guild: discord.Guild):
        color = self.embed_color(guild)
        embed = discord.Embed(
            title=":fire: " + data['title'],
            description="This week's affixes.",
            colour=color
        )
    ...
    @staticmethod
    def seconds_from_now(epoch: int):
        now = datetime.datetime.utcnow()
